...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map((element) => {
    return element.filter(innerelement => typeof (innerelement) === 'number' && innerelement % 5 === 0).map(innerelement => Math.pow(2, innerelement));
  });
};

// Recieved TA help from Tammy Do :)
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findShortest = (data) => {
  let height = [];
  data.forEach(element => {
    height.push(parseInt(element.height));
  });
  let lowestNum = height.reduce((curr, prev) => {
    return prev < curr ? prev : curr;
  });
  let name = '';
  data.forEach(element => {
    if (element.height === lowestNum.toString()) {
      name = element.name;
    }
  });
  return name;
