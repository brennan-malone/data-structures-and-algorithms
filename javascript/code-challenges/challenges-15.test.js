...
const screenForNames = (arr) => {
  let cleanedNames = [];
  let regex = /((^[MD]+[rs]+\.))\s{1}(\w+)/;
  arr.forEach(element => {
    let result = regex.test(element);
    if (result) cleanedNames.push(element);
  });
  return cleanedNames;
};
...
const toTitleCase = (arr) => {
  let newArr = arr.map((element) => {
    let modifiedStr = `${element[0].toUpperCase()}${element.substring(1)}`;
    return modifiedStr;
  });
  return newArr;
};
...
const detectTicTacToeWin = (board) => {
  let result = false;
  if (areEqual(board[0][0], board[0][1], board[0][2])) return true;
  if (areEqual(board[1][0], board[1][1], board[1][2])) return true;
  if (areEqual(board[2][0], board[2][1], board[2][2])) return true;
  if (areEqual(board[0][0], board[1][0], board[2][0])) return true;
  if (areEqual(board[0][1], board[1][1], board[2][1])) return true;
  if (areEqual(board[0][2], board[1][2], board[2][2])) return true;
  if (areEqual(board[0][0], board[1][1], board[2][2])) return true;
  if (areEqual(board[0][2], board[1][1], board[2][0])) return true;
  console.log(result);
  return result;
};
