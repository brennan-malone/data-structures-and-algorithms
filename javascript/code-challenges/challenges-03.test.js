...
const typeNum = (arr) => {
  let number = arr.filter((element) => !isNaN(element));
  return number;
};
...
const containsAnd = (arr) => {
  let strArr = arr.filter(element => element.includes('and'));
  return strArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let noDupes = arr.filter(element => !forbiddenValues.includes(element));
  return noDupes;
};
